...
const PropertyCard = ({ property, rentalType = "short" }) => {
  ...
  return (
    <div className="group relative border border-espresso-line bg-linen transition-colors duration-500 hover:border-bronze/40 overflow-hidden group relative flex flex-col h-full">
      {/* Favorite badge */}
      {isGuestFavorite && (
        <span className="absolute top-3 left-3 z-10 bg-cream/85 text-bronze text-[10px] font-semibold tracking-wider uppercase px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1">
      ...
      <button className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-cream/80 hover:bg-linen flex items-center justify-center shadow-sm transition-all group/heart cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-espresso-soft/55 group-hover/heart:text-bronze transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
        </svg>
      </button>
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        {/* Location */}
        <p className="flex items-center gap-1 text-xs text-espresso-soft/75 mb-2">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-bronze" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <Link to={`/property/${id}`} className="cursor-pointer">
          <h3 className="font-semibold text-espresso text-sm leading-snug group-hover:text-bronze transition-colors line-clamp-1 mb-3">
            {title}
          </h3>
        </Link>

        {/* Specs (smartLET style — icon + text) */}
        <div className="flex gap-4 text-xs text-espresso-soft/55 mt-auto pt-3 border-t border-espresso-line">
